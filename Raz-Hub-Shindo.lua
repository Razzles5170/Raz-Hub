	...
	do
		Webhook:AddTextBox("Webhook Url", webhook_settings.url, function(text)
			if (webhook_settings.url ~= text) then
				...
				webhook_main:send("Raz Hub Logger - TEST", "This is a test lol", "Shindo Life • REWRITE")
			end
		end)
		...
		local Selection = Webhook:AddDropdown("Logs Selection", nil, true, function(option)
			...
			if (config.webhook.reminder) then
				webhook_main:send("Raz Hub Logger", ("**%s** has been turned %s"):format(tostring(option), config.webhook[option:lower()] and 'on' or 'off'), 'Shindo Life • REWRITE')
			end
		end)
